...
- Файлы хранятся приватно (`verify_media_dir`, не раздаются статикой) и используются
  только для проверки.
- Когда оба файла загружены — заявка попадает в очередь модерации:
  `GET /v1/moderation/verifications`, решение `POST .../{id}/decision`
  (approved выставляет бейдж `is_verified`).
- `GET /v1/verify/status` — статус процесса (шаги, submitted, решение).

### Статусы сообщений
- `sent` → `delivered` (получатель загрузил историю) → `read` (отметил прочтение);
  время — `created_at` в каждом сообщении.

### Видеозвонки: доступность и приватность
- Настройка `video_calls_enabled` в анкете (`PUT /v1/profile`) — запрет входящих звонков.
- Онлайн-статус: активность в пределах `online_window_sec` (300 c).
- Состояние кнопки видео отдают `GET /v1/discovery` (`video_state`) и
  `GET /v1/matches` (`other_video_state`): `available | offline | disabled`.
- `POST /v1/matches/{id}/video` → `409 video_unavailable {reason}` если получатель
  офлайн или запретил звонки.

### Переписка с карточки
- `POST /v1/discovery/message` — первое сообщение без взаимного лайка: создаёт
  диалог (`matches.origin="direct"`) и сообщение; работают блокировки, антиспам
  и rate-limit; получателю уходит уведомление.

### Решение по email-флоу (E7)
Поле `users.email` (зашифровано в покое) **оставлено намеренно**: оно потребуется
на Стадии 4 для отправки фискальных чеков (54-ФЗ) и восстановления доступа.
Вход по email/паролю не реализуется — аутентификация только по SMS-OTP.

### Регистрация и согласия (152-ФЗ) — детали
- `GET /v1/legal/documents` — каталог документов с версиями: согласие на обработку
  ПДн и пользовательское соглашение/оферта (обязательные), рассылки (по желанию).
- `POST /v1/auth/verify-otp` при регистрации нового пользователя требует
  `accepted_documents` с обязательными типами (`privacy`, `terms`); иначе `400`
  с `{"error":"consent_required","missing":[...]}`.
- Принятые согласия фиксируются в `consents` с типом, версией и таймстампом.
...
## Что сделано на Стадии 3.3 (стабилизация и открытая бета) — Стадия 3 завершена
- **Уведомления**: регистрация устройств `POST /v1/devices`, in-app лента
  `GET /v1/notifications`, отметка прочтения; push-доставка — заглушка под FCM/RuStore.
- Привязка к событиям: мэтч, новое сообщение, входящий видеозвонок.
- Модели `notifications`/`device_tokens` + миграция 0010.
- Чек-лист безопасности и 152-ФЗ: `docs/security/security-152fz.md`.

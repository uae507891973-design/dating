...
## Что сделано на Стадии 3.1 (антифрод-движок)
- Поведенческие сигналы: частота сообщений/лайков, повторяющиеся (шаблонные)
  сообщения, жалобы на пользователя; митигация для верифицированных.
- Внутренний **trust-score** (0..100, не публичная шкала) на пользователе.
- Авто-флаг при превышении порога риска → очередь модерации; авто-оценка после
  отправки сообщений.
- Эндпоинты модератора: `GET /v1/moderation/flags`, `POST .../flags/{id}/resolve`,
